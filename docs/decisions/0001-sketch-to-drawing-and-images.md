...
- IA (geração a partir do esboço):
  - Contrato `ImageGenerator` com método `generateFromImage(inputUrl, prompt?)`.
  - Providers disponíveis: `OpenAIImageGenerator` e `StabilityImageGenerator`.
  - Seleção por factory (`IMAGE_PROVIDER=openai|stability`), com OpenAI como default.
  - Ambos retornam `Buffer` para upload via `ImageService.createFromUpload`.
...
- Permite rollout progressivo e rollback rápido de provider de IA apenas por env var.
...
- Drawing: `src/modules/drawing/**`, `src/infrastructure/ai/openai-image-generator.ts`, `src/infrastructure/ai/stability-image-generator.ts`, `src/infrastructure/ai/image-generator-factory.ts`
- App: `src/app.ts`
