...
- **`DrawingService`**: `create` persiste sem checar se `sketchId` aponta para um sketch existente; `update` só garante que o **drawing** exista, não valida sketch ao alterar `sketchId`. `generateFromSketch(sketchId, prompt?)` carrega o sketch com `SketchService.getByIdOrThrow`, resolve `mediaId` do sketch para id numérico de `Image`, obtém a URL com `ImageService.getByIdOrThrow`, chama `OpenAIImageGenerator.generateFromImage`, grava a saída com `ImageService.createFromUpload` e por fim faz **`repo.create`** (novo drawing), preenchendo `mediaId` com o id da imagem gerada e `sketchId` com o id do sketch.
...
- **`OpenAIImageGenerator`**: implementação placeholder documentada no código: `fetch(inputUrl)` e retorno do buffer da imagem original; não usa `OPENAI_API_KEY` nem API de geração/edição. A interface `ImageGenerator` expõe apenas `generateFromImage(inputUrl, prompt?)`.
...
- IDs de imagem no domínio costumam trafegar como **string** em `Sketch`/`Drawing` (`mediaId`, `sketchId`), com parsing numérico onde necessário (vide `SketchService.assertMediaExistsIfProvided` e `generateFromSketch` para `mediaId` do sketch).
...
- **Serviço `generateFrom*`**: deixar claro no nome (ex.: `generateImageForDrawing`) que atualiza um drawing existente; documentar que `mediaId` anterior (se houver) permanece no storage até política de limpeza existir.
- **OpenAI**: implementar `generateFromImage` com o SDK oficial ou `fetch` para o endpoint escolhido; usar **`/images/edits`** se o produto for “refinar rascunho com prompt”; mapear `prompt` opcional para string vazia ou default de produto. Garantir que o buffer retornado corresponda a um MIME permitido por `ImageService.createFromUpload` ou converter antes do upload.
...
- `src/modules/drawing/drawing-service.ts` — CRUD, `generateFromSketch`, integração com sketch/image/generator.
...
- `src/infrastructure/ai/openai-image-generator.ts` — placeholder e interface `ImageGenerator`.
